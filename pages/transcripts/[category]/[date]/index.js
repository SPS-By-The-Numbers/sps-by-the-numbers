...
export default function Index(props) {
    ...
    const videoLinks = videos.map(
        video => <li key={video.metadata.video_id} className="mx-3 list-disc"><Link href={ getTranscriptPath(category, date, video.metadata.video_id) }>{video.metadata.title}</Link></li>
    );
    return (
